...
@section('content')
            ...
            @foreach ($featuredNews as $featured)
                <a href="{{ route('news.show', $featured->slug) }}">
                    <div class="border border-slate-200 p-3 rounded-xl hover:border-primary hover:cursor-pointer transition duration-300 ease-in-out relative"
                        style="height: 100%;">
                        <!-- Kategori -->
                        <div class="bg-primary text-white rounded-full w-fit px-5 py-1 font-normal text-sm mb-3">
                            {{ $featured->newsCategory->name }}
                        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            <div
                ...
                <a href="{{ route('news.show', $news[0]->slug) }}">
                    ...
                    <p class="text-slate-400 text-base mt-1">
                        {{ \Carbon\Carbon::parse($news[0]->created_at)->format('d M Y') }}</p>
                </a>
            </div>
            <!-- Berita 2 -->
            ...
            @foreach ($authors as $author)
                <a href="{{ route('authors.show', $author->username) }}">
                    <div
                        ...
                        <p class="font-bold text-xl mt-4">{{ $author->username }}</p>
    ...
    <div class="flex flex-col px-14 mt-10 mb-10">
        ...
        <div class="grid sm:grid-cols-1 gap-5 lg:grid-cols-4">
            @foreach ($news as $choiceAuthor)
                <a href="{{ route('news.show', $choiceAuthor->slug) }}">
                    <div class="border border-slate-200 p-3 rounded-xl hover:border-primary hover:cursor-pointer transition duration-300 ease-in-out"
                        ...
                        <div
                            ...
                            {{ $choiceAuthor->newsCategory->name }}</div>
                        <img src="{{ asset('storage/' . $choiceAuthor->thumbnail) }}" alt=""
                            class="w-full rounded-xl mb-3" style="height: 200px; object-fit: cover;">
                        <p class="font-bold text-base mb-1">{{ $choiceAuthor->title }}</p>
                        <p class="text-slate-400">{{ \Carbon\Carbon::parse($choiceAuthor->created_at)->format('d M Y') }}
                        </p>
                    </div>
                </a>
            @endforeach
